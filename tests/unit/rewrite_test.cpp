...
TEST(Rewrite, SimpleRewrite) {
    ...
    Rewriter rewriter(egraph, rules, EGraphConfig{.node_limit = 100});
...
TEST(Rewrite, SolveRule) {
    ...
    Rewriter rewriter(egraph, {solver_left}, EGraphConfig{.node_limit = 100});
    bool changed = rewriter.apply_rewrites();
    EXPECT_TRUE(changed);
...
TEST(Rewrite, SolR_RightSolve) {
    ...
    Rewriter rewriter(egraph, {solver_right, trsm_rn}, EGraphConfig{.node_limit = 100});
    bool changed = rewriter.apply_rewrites();
    EXPECT_TRUE(changed);
...
TEST(Rewrite, LLtRewrite) {
    ...
    Rewriter rewriter(egraph, {llt_invert}, EGraphConfig{.node_limit = 100});
    bool changed = rewriter.apply_rewrites();
    EXPECT_TRUE(changed);
...
TEST(Rewrite, LLtToUtURewrite) {
    ...
    Rewriter rewriter(egraph, {llt_to_utu}, EGraphConfig{.node_limit = 100});
    bool changed = rewriter.apply_rewrites();
    EXPECT_TRUE(changed);
...
TEST(Rewrite, BackoffScheduler) {
    ...
    Rewriter rewriter(egraph, rules, EGraphConfig{.node_limit = 1000, .enable_backoff = true});
