...
TEST(ScaleRewrite, ScaleCollapseDoubles) {
    ...
    Rewriter rewriter(egraph, {scale_collapse}, EGraphConfig{.node_limit = 100});
...
TEST(ScaleRewrite, ScaleCombineDoubles) {
    ...
    Rewriter rewriter(egraph, {scale_combine, scale_one}, EGraphConfig{.node_limit = 100});
    rewriter.apply_rewrites();
...
TEST(ScaleRewrite, ScaleCombineImplicitDoubles) {
    ...
    Rewriter rewriter(egraph, {scale_combine_implicit}, EGraphConfig{.node_limit = 100});
    rewriter.apply_rewrites();
...
TEST(ScaleRewrite, ScaleInverse) {
    ...
    Rewriter rewriter(egraph, {scale_inverse}, EGraphConfig{.node_limit = 100});
    rewriter.apply_rewrites();
...
TEST(ScaleRewrite, ComplexScaleProduct) {
    ...
    Rewriter rewriter(egraph, {scale_inverse, scale_mul_distribute_left, scale_mul_distribute_right, scale_collapse, scale_one}, EGraphConfig{.node_limit = 100});
    rewriter.apply_rewrites();
