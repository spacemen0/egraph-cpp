...
static PropertyTable get_property_table() {
    ...
    pt.add_or_update_property_entry("X", {.shape = std::make_pair(100, 100), .flags = {.is_positive_definite = true}});
    ...
    pt.add_or_update_property_entry("y", {.shape = std::make_pair(100, 1)});
    pt.add_or_update_property_entry("M", {.shape = std::make_pair("A", "B"), .flags = {.is_positive_definite = true}});
