...
inline int LAPACKE_dormqr(int matrix_layout, char side, char trans, int m, int n, int k, const double* a, int lda, const double* tau, double* c, int ldc) {
    if (matrix_layout != LAPACK_COL_MAJOR) return -1;
    int info = 0;
    ...
    double wkopt = 0.0;
    // workspace query
    dormqr_(&side, &trans, &m, &n, &k, const_cast<double*>(a), &lda, const_cast<double*>(tau), c, &ldc, &wkopt, &lwork, &info);
    lwork = (int)wkopt;
    double* work = new double[lwork];
    dormqr_(&side, &trans, &m, &n, &k, const_cast<double*>(a), &lda, const_cast<double*>(tau), c, &ldc, work, &lwork, &info);
    delete[] work;
    return info;
}
